...
async def check_code_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    while True:  # حلقه نامحدود چک کردن کد
...
async def search_number(user_id, chat_id, msg_id, code, site, context):
    while True:
        ...
        if valid:
            user_sessions[user_id] = (id_, site, number)
            buttons = [
...
async def main():
    ...
    application.add_handler(CallbackQueryHandler(cancel_number_callback, pattern=r"^cancel_number$"))
    application.add_handler(CallbackQueryHandler(check_code_callback, pattern=r"^checkcode$"))
