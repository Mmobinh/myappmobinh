...
async def get_number_tiger(code):
    url = f"https://api.tiger-sms.com/stubs/handler_api.php?api_key={API_KEY_TIGER}&action=getNumber&service={SERVICE}&country={code}&ref=$ref&maxPrice=60&providerIds=55,188.234"
    async with aiohttp.ClientSession() as s:
        async with s.get(url) as r:
            return await r.text()
