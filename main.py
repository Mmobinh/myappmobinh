...
async def search_number(user_id, chat_id, msg_id, code, site, context):
    ...
    while user_id not in cancel_flags:
        ...
        if valid:
            ...
            asyncio.create_task(start_timer(context, chat_id, msg.message_id, number, 25*60))
...
async def start_timer(context, chat_id, msg_id, number, total_seconds):
    for remaining in range(total_seconds, -1, -1):
        mins, secs = divmod(remaining, 60)
        time_str = f"{mins:02}:{secs:02}"
        try:
            await context.bot.edit_message_text(
                f"📱 شماره سالم: <code>{number}</code>\n⏳ زمان باقی‌مانده: {time_str}",
                chat_id=chat_id,
                message_id=msg_id,
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
