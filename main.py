...
await query.answer()
site = query.data.split("")[1]
countries = COUNTRIES.get(site,{})
country_buttons = [InlineKeyboardButton(name, callback_data=f"country{site}{id}") for name,id_ in countries.items()]
...
site, code = query.data.split("")[1], query.data.split("")[2]
...
if rec[0] == id:
, site, number, msg_id = rec
resp = await get_code(site, id)
if resp.startswith("STATUS_OK:"):
code = resp[len("STATUS_OK:"):].strip()
await context.bot.edit_message_text(
f"📩 کد برای شماره <code>{number}</code> دریافت شد:\n<code>{code}</code>",
